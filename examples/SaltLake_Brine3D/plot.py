...
from modflow_model import MINERAL_MOLAR_VOLUMES_L_PER_MOL, PROFILES
from validate import run_label, summarize_run
...
def read_run(profile: str, scenario: str) -> dict[str, Any]:
    ...
    times_years = np.load(output_dir / "results_times.npy") / metadata["time"]["days_per_year"]
    ...
    if (output_dir / "results_K.npy").is_file():
        hydraulic_k = np.load(output_dir / "results_K.npy").reshape((times_years.size, *shape))
    else:
        hydraulic_k = np.broadcast_to(initial_k, (times_years.size, *shape)).copy()

    initial_br = float(metadata["initial_selected_output"]["Br"])
    channel_br = float(metadata["channel_components_mol_per_litre"]["Br"])
    tracer = np.clip(
        (results[:, field_index["Br"], :] - initial_br) / (channel_br - initial_br),
        0.0,
        1.0,
...
def read_specific_discharge(
    ...
    for time_index, time_days in enumerate(times_days):
        ...
        if node_indices.size != nxyz or node_indices.min() != 0 or node_indices.max() != nxyz - 1:
...
def first_arrival_time(tracer: np.ndarray, times_years: np.ndarray, threshold: float) -> np.ndarray:
...
def effective_flow_width_m(
    ...
    positive_x_flux_by_row = np.sum(
        np.maximum(qx[:, :, column], 0.0) * layer_thicknesses_m[:, None] * delc_m,
        axis=0,
    )
...
def effective_flow_width_profile_m(
    ...
    return np.asarray(
        [
            effective_flow_width_m(qx, layer_thicknesses_m, delc_m, column)
...
def plot_channels(profile: str) -> None:
    configure_matplotlib()
    feedback = read_run(profile, "feedback")
    fixed = read_run(profile, "fixed")
    ...
    comparison_dir = runtime_path(__file__, "output") / f"{profile}_channel_comparison"
    ...
    layer_means = np.asarray(
        metadata["initial_hydraulic_conductivity"]["layer_geometric_means_m_per_day"],
        dtype=float,
    )
    log_relative_k = np.log(feedback["initial_k"] / layer_means[:, None, None])
    ...
    x_values = np.linspace(float(np.min(log_relative_k)), float(np.max(log_relative_k)), 300)
    ...
    layer_thicknesses = np.asarray(metadata["grid"]["layer_thicknesses_m"], dtype=float)
    ...
    for flow_index, time_years in enumerate(feedback["flow_times_years"]):
        ...
        for name, run in (("feedback", feedback), ("fixed", fixed)):
            q_magnitude_layer1 = np.hypot(run["qx"][flow_index, 0], run["qy"][flow_index, 0])
    ...
    feedback_width = np.asarray([row["effective_flow_width_feedback_m"] for row in metric_rows])
    fixed_width = np.asarray([row["effective_flow_width_fixed_m"] for row in metric_rows])
    feedback_top10 = np.asarray([row["top10_q_share_feedback"] for row in metric_rows])
    fixed_top10 = np.asarray([row["top10_q_share_fixed"] for row in metric_rows])

    # Figure 2: evolution of permeability, tracer and flow focusing.
    target_years = (10.0, 20.0, 30.0)
    result_indices = [
        int(np.argmin(np.abs(feedback["times_years"] - target))) for target in target_years
    ]
    flow_indices = [
        int(np.argmin(np.abs(feedback["flow_times_years"] - target))) for target in target_years
    ]
    ...
    for flow_index in flow_indices:
        q_feedback = np.hypot(feedback["qx"][flow_index, 0], feedback["qy"][flow_index, 0])
        q_fixed = np.hypot(fixed["qx"][flow_index, 0], fixed["qy"][flow_index, 0])
        floor = max(float(np.quantile(q_fixed, 0.02)), 1.0e-12)
        q_log_ratios.append(np.log10((q_feedback + floor) / (q_fixed + floor)))
    q_limit = max(
        0.05,
    ...
    for column, (target, result_index, _flow_index) in enumerate(
    ...
    figure.colorbar(k_image, ax=axes[0, :], shrink=0.75, label="水力传导系数倍率")
    figure.colorbar(tracer_image, ax=axes[1, :], shrink=0.75, label="Br 补给水比例")
    figure.colorbar(q_image, ax=axes[2, :], shrink=0.75, label="流速反馈放大（log10）")
    ...
    fixed_arrival = first_arrival_time(fixed["tracer"], fixed["times_years"], threshold=0.05)
    ...
    final_q_fixed = np.hypot(fixed["qx"][final_flow_index, 0], fixed["qy"][final_flow_index, 0])
    q_floor = max(float(np.quantile(final_q_fixed, 0.02)), 1.0e-12)
    final_q_amplification = np.log10((final_q_feedback + q_floor) / (final_q_fixed + q_floor))
    ...
    figure, axes = plt.subplots(2, 2, figsize=(14, 8), constrained_layout=True)
    axes[0, 0].plot(feedback["times_years"], feedback["well_tracer"], label="feedback", lw=2)
    axes[0, 0].plot(fixed["times_years"], fixed["well_tracer"], "--", label="fixed", lw=2)
    ...
    initial_k_concentration = float(metadata["initial_selected_output"]["K"])
    axes[0, 1].plot(
    ...
    axes[1, 1].plot(flow_times, 100.0 * feedback_top10, label="feedback", lw=2)
    axes[1, 1].plot(flow_times, 100.0 * fixed_top10, "--", label="fixed", lw=2)
    axes[1, 1].set(
    ...
    common_arrival = np.isfinite(feedback_arrival_layer1) & np.isfinite(fixed_arrival_layer1)
    ...
    feedback_breakthrough_50 = feedback["summary"]["bromide_breakthrough_50pct_years"]
    fixed_breakthrough_50 = fixed["summary"]["bromide_breakthrough_50pct_years"]
    ...
    final_width_reduction = float(100.0 * (fixed_width[-1] - feedback_width[-1]) / fixed_width[-1])
    top10_flow_share_increase = float(feedback_top10[-1] - fixed_top10[-1])
    ...
    summary = {
        ...
        "maximum_flow_width_reduction_percent_between_channel_and_wells": float(
            np.nanmax(width_reduction_percent[channel_column : well_column + 1])
        ),
        ...
        "maximum_K_multiplier_feedback": float(feedback["summary"]["maximum_K_multiplier"]),
        "layer1_area_fraction_K_multiplier_gt_2": float(np.mean(k_ratios[-1, 0] > 2.0)),
        "median_arrival_advance_years_where_both_arrive": (
            float(np.median(arrival_advance[common_arrival])) if np.any(common_arrival) else None
        ),
        "feedback_peak_well_K_mol_per_L": float(feedback["summary"]["peak_well_K_mol_per_L"]),
        "fixed_peak_well_K_mol_per_L": float(fixed["summary"]["peak_well_K_mol_per_L"]),
        "final_mean_well_head_feedback_m": float(feedback["summary"]["mean_final_well_head_m"]),
        "final_mean_well_head_fixed_m": float(fixed["summary"]["mean_final_well_head_m"]),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--channels", action="store_true", help="plot paired preferential-channel diagnostics"
    )
...
def main() -> None:
    ...
    np.load(output_dir / "initial_porosity.npy")
    np.load(output_dir / "initial_K.npy")
