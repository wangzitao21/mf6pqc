...
from validate import PROFILE_X, grid_centres, load_case_fields
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=runtime_path(__file__, "output") / "PHT3D_E11_comparison.png",
...
def main() -> None:
    ...
    profile_layers = (z >= 32.5) & (z <= 34.0) & wet[:, well_column]
    ...
    for panel, (ax, spec) in enumerate(zip(axes.flat, PLOT_SPECS, strict=False)):
        ...
        official_profile_x = PROFILE_X + profile_width * (official_profile - low) / (high - low)
        model_profile_x = PROFILE_X + profile_width * (model_profile - low) / (high - low)
        ax.plot(
        ...
        colorbar = fig.colorbar(contour, ax=ax, ticks=ticks, fraction=0.050, pad=0.045)
