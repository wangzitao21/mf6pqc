...
def run_method(method: str) -> np.ndarray:
    root = runtime_path(__file__, "simulation") / "reaction_only" / method.lower()
    output = runtime_path(__file__, "output") / "reaction_only" / method.lower()
    simulator = MF6PQC(
    ...
    try:
        ...
        return (
            np.asarray(simulator.modflow_api.get_value(address), dtype=float).copy()
            / inlet_concentration
        )
    finally:
        simulator.finalize()
...
def _persist(method: str, profile: np.ndarray) -> None:
    directory = runtime_path(__file__, "output") / "reaction_only" / method.lower()
...
def _launch(method: str) -> np.ndarray:
    ...
    return np.load(
        runtime_path(__file__, "output") / "reaction_only" / method.lower() / "profile.npy"
    )
...
def run_check() -> None:
    ...
    backend_error = max(
        float(np.max(np.abs(profile - analytical))) for profile in profiles.values()
    )
    report = {
        ...
        "method_means": {method: float(np.mean(profile)) for method, profile in profiles.items()},
    ...
    output = runtime_path(__file__, "output") / "reaction_only"
    (output / "validation.json").write_text(
        json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print(json.dumps(report, indent=2, sort_keys=True))
    if pairwise_max > 1.0e-10:
        raise AssertionError("SNIA, Strang, and SIA disagree in the reaction-only invariant")
