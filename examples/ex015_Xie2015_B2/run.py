"""Run the ex015_Xie2015_B2 reactive-transport benchmark."""
...
import numpy as np
from ex015_Xie2015_B2.modflow_model import build_model
...
def main() -> None:
    ...
    simulation_config = SimulationConfig(
        ...
        nxyz=NXYZ,
        nthreads=1,
        backend_factory=process_backend(24),
        paths=BackendPaths(
        ...
        output=OutputOptions(save_interval=1250, save_interval_offset=1),
    )
    ...
    with MF6PQC.from_config(simulation_config) as simulator:
        ...
        required_outputs = require_output_files(
            output_dir, ("results.npy", "results_porosity.npy", "results_K.npy")
        )
        print("MF6PQC reactive outputs saved successfully:")
        for path in required_outputs:
            print(f"  {path}  shape={np.load(path, mmap_mode='r', allow_pickle=False).shape}")
        print("ex015 done.")
