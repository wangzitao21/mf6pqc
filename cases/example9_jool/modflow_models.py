...
def transport_model(
    ...
    idomain = np.genfromtxt("./examples/example9_jool/input_data/idomain.txt", delimiter='\t')
    ...
    k11 = np.genfromtxt("./examples/example9_jool/input_data/K11.txt", delimiter='\t')
    ...
    chd_data = np.loadtxt("./examples/example9_jool/input_data/chd.txt")
    chd_data = chd_data[:, [0, 1, 2]] - 1
    chd_spd = []
    for i in np.arange(chd_data.shape[0]):
        chd_spd.append([tuple(chd_data[i].astype(int)), 60.0,])
    ...
    lake_data = np.loadtxt("./examples/example9_jool/input_data/lake.txt")
    lake_data = lake_data[:, [0, 1, 2]] - 1
    lake_spd = []
    for i in np.arange(lake_data.shape[0]):
        lake_spd.append([tuple(lake_data[i].astype(int)), 20.0, 1198.844, *bc])
    # lake_spd = np.array(lake_spd, dtype=object)
    wel_spd = lake_spd
